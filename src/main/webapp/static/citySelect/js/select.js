//引入新的插件
layui.config({
    base: '../static/citySelect/js/'//拓展模块的根目录
}).extend({
    pca: 'pca'
});
...
layui.use(['form', 'layedit', 'laydate', 'upload', "jquery", "pca"], function () {
    var $ = layui.$
        ...
        , pca = layui.pca;
    //带初始值进行初始化
    // pca.init('select[name=P1]', 'select[name=C1]', 'select[name=A1]', '江苏', '南京', '江宁区');
    ...
    var splitAddress = userAddress.split('-');
    var province = splitAddress[0].trim();
    var city = splitAddress[1].trim();
    console.log("province = " + province + " city = " + city);
    pca.init('select[name=P1]', 'select[name=C1]', 'select[name=A1]', province, city);
